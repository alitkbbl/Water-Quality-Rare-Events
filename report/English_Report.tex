...
\begin{document}
	...
	\begin{figure}[H]
		...
		\caption{Comparison of weight distributions under different proposal distribution tail-thickness settings and their effect on the stability of the IS estimator.}
		\label{fig:weight-distributions}
	\end{figure}
	
	\textbf{Analysis of ESS results:}
	
	\begin{itemize}
		\item \textbf{Lighter tail ($\sigma_q = 0.5\sigma$):} The ESS value drops sharply, and the ratio $ESS/N$ reaches the negligible value $0.0002$. This shows that the entire credibility of the estimate rests on only a few samples with very large weights, and severe instability has occurred.
		\item \textbf{Heavier tail ($\sigma_q = 1.5\sigma$):} In this case, the function $q(x)$ keeps the denominator large, the weights remain bounded and small, and the ratio $ESS/N$ is much more desirable, namely $0.0170$. This proves that, for protection against variance explosion, the distribution $q$ should be chosen wider than $f$.
	\end{itemize}
